...
def check_run_result(expected, actual):
  ...
  if (expected_value, expected_type) != (actual_value, actual_type):
    raise TestingError((expected_value, expected_type), (actual_value, actual_type))
...
def run_stimpl_sanity_tests():
  try:
    ...
    program = Program(Assign(Variable("i"), FloatingPointLiteral(1.0)), Variable("i"))
    check_run_result((1, FloatingPoint(), None), run_stimpl(program))

    program = Program(Assign(Variable("i"), StringLiteral("test")), Variable("i"))
    check_run_result(("test", String(), None), run_stimpl(program))

    program = Program(Assign(Variable("i"), BooleanLiteral(True)), Variable("i"))
    ...
    program = Multiply(FloatingPointLiteral(5.5), FloatingPointLiteral(2.0))
    check_run_result((11.0, FloatingPoint(), None), run_stimpl(program))

    program = Divide(IntLiteral(10), IntLiteral(10))
    check_run_result((1, Integer(), None), run_stimpl(program))

    program = Divide(FloatingPointLiteral(10.0), FloatingPointLiteral(20.0))
    ...
    # j = 11
    # result = 10 + (10 + 11) = 31
    program = Add(Assign(Variable("i"),IntLiteral(10)), Add(Variable("i"), Assign(Variable("j"), IntLiteral(11))))
    run_value, run_type, run_state = run_stimpl(program)
    ...
    # j = 11
    # result = 10 - (10 + 11) = -11
    program = Subtract(Assign(Variable("i"),IntLiteral(10)), Add(Variable("i"), Assign(Variable("j"), IntLiteral(11))))
    run_value, run_type, run_state = run_stimpl(program)
